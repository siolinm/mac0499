\begin{figure}[H]
    ...
    \begin{tikzpicture}[baseline=-2cm,scale=0.8]
        ...
        \draw[pred, blue, dashdotted] (3) edge[out=190,in=120]
        node[blue, above=10pt] {$1$} (5);
        \draw[sucessor] (5) edge[out=0,in=260] (3);
        \draw[pred, blue, dashdotted] (5) edge[out=170,in=285]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        $i$ & $x_0$ & $v$                   & $\cert[i]$                  \\
        \hline
        $1$ & $6$   & \textcolor{blue}{$5$} & \textcolor{blue}{$+\infty$} \\

        $2$ & $3$   & $5$                   & $+\infty$                   \\

        $3$ & $2$   & $1$                   & $2$                         \\

        $4$ & $7$   & $4$                   & \textcolor{blue}{$1$}       \\
        ...
        \hline
    \end{tabular}
    \caption[Exemplo de mudanças na árvore binária de busca após operação \textsc{change}]{Após chamar
        { \normalfont \textsc{change}$(1, 5)$}, a velocidade do elemento $1$ foi
    alterada para $5$ e os certificados do elemento $1$ e do
    elemento $4$ foram atualizados.}
    \label{fig:abb:change}
\end{figure}