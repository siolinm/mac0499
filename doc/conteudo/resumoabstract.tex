...
estruturas serão orientadas a eventos, que são o vencimento de certificados.
Cada certificado será colocado numa fila de prioridades com o seu prazo de
validade como prioridade. O vencimento de um certificado significa que a
estrutura ficou inválida e é necessário realizar mudanças para que ela volte a
se tornar válida, removendo os certificados vencidos e gerando novos
...
attribute about these objects at the current moment. For example, query the
closest pair of points at the moment, in a set of moving points.

The proposed interface for the data structures supports three operations: query,
that returns the maintained attribute, advance, that sets the current moment,
and changing the object motion, given by updates of a function called flight
plan. The flight plan is necessary to maintain the attribute of interest over
the elements, he defines the objects motion through time and will be used for
computing the certificates. The certificates are objects that ensure the
internal state of our data structure is correct until a certain instant of time,
the certificate's expiral time. Our data structures will be event driven where
the events are the expiry of certificates. Each certificate will be put in a
priority queue with his expiral time as priority. The expiration of a
certificate means the data structure has become invalid and we need to change it
in order to be correct again, deleting the expired certificates and generating
new ones in the process.

The traditional way of analyzing algorithms does not work well with these
structures. Because of that, Basch, Guibas and Hershberger also proposed four
criteria to determine the quality of the structures: responsiveness, efficiency,
locality and compactness.

In this work, we will be studying four problems: sorting, maximum, closest pair
and delaunay triangulation, all in a kinetic context. We will also be studying
the respective kinetic data structures used to solve these problems and will
discuss about the quality criteria in each one of them. For some of the
structures we will also consider a kinetic-dynamic scenario, where insert and
delete operations will be supported in a kinetic context.
